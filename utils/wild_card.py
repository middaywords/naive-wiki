...
class autocorrectWC():
    ...
    def wild_card(self, term: str):
        ...
        if case != 4:
            term_list = self.prefix_match(self.permuterm, query)

        elif case == 4:
            # queryA Z$X
            term_listA = self.prefix_matchA(self.permuterm, queryA)

            # queryB Y
            term_list = self.prefix_matchB(self.permuterm, queryB, term_listA)
